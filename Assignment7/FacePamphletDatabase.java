...
public class FacePamphletDatabase implements FacePamphletConstants {
	...
	private HashMap<String, FacePamphletProfile> database = new HashMap<>();
	...
	public void deleteProfile(String name) {
		if (containsProfile(name)) {
			Iterator<String> iterator = database.get(name).getFriends();
			while (iterator.hasNext()) {
				database.get(iterator.next()).removeFriend(name);
			}
